...
bool DJSession::load_track_to_mixer_deck(const std::string &track_title)
{
    ...
    int num = mixing_service.loadTrackToDeck(*track);
    if (num == 0)
    {
        stats.deck_loads_a++;
        stats.transitions++;
...
void DJSession::simulate_dj_performance()
{
    ...
    std::vector<std::string> auto_playlists;
    size_t auto_index = 0;
    ...
    if (play_all)
    {
        ...
        {
            auto_playlists.push_back(pair.first);
        }
        std::sort(auto_playlists.begin(), auto_playlists.end());
    }
    ...
    while (true)
    {
        ...
        if (play_all)
        {
            if (auto_index >= auto_playlists.size())
                break;
            current_playlist_name = auto_playlists[auto_index];
            auto_index++;
        }
        else
        {
        ...
        library_service.loadPlaylistFromIndices(current_playlist_name, session_config.playlists[current_playlist_name]);
